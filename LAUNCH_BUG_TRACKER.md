...
| LB-030 | 🔴 P0 | 🟢 Fixed | My Garden (empty) | Both | Fresh account | Open My Garden tab with zero saved plants | Attractive empty state, not an empty garden UI | Empty state existed but copy/CTA didn't match the requested spec | Empty-state copy predated the spec | _(this round — see commit below)_ | — | Home (first-run hero) and My Garden already gate the standard garden UI behind `plants.length`. Aligned the My Garden empty state to spec: "Your garden is waiting 🌱" / "Scan your first plant to start growing your digital garden." / CTA **Scan Your First Plant** → Scan tab. **Verify on device** |
