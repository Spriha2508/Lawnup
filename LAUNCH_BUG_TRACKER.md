...
|---|---|---|---|---|---|---|---|
| LB-029 | 🔴 P0 | Auth (Google) | Google Sign-In still fails during onboarding | **CONFIRMED (full-flow trace): SHA-1 mismatch.** The app signs **both debug *and* release** with `android/app/debug.keystore` (SHA-1 `5E:8F:16:…F6:25`), but `google-services.json` registers the Android OAuth client against `certificate_hash 5fbc5bc4…` — a *different* key. Google's OAuth backend therefore returns `DEVELOPER_ERROR` (code 10). Verified the rest of the flow is **correct**: Web client ID matches `client_type:3`, `configureGoogleSignIn()` runs at boot in the real `App.tsx`, `applicationId`=`com.lawnup.app`=`package_name`, gms plugin applied, dependency present, JS flow sound. So it is a SHA issue — but only after ruling everything else out. | **Owner/Firebase action (cannot be done in-repo — server-side check):** register the `android/app/debug.keystore` SHA-1 `5E:8F:16:06:2E:A3:CD:2C:4A:0D:54:78:76:BA:A6:F3:8C:AB:F6:25` (+ SHA-256 `FA:C6:17:…3B:9C`) under Firebase → Project Settings → Android app (`com.lawnup.app`) → SHA fingerprints, **re-download `google-services.json`**, rebuild. If distributing via EAS, also register that keystore's SHA (`eas credentials -p android`). | 🟡 Implemented (code+docs) | Re-open of Round 1 LB-029; **BLK-1**. In-repo this round (`<hash>`): actionable `DEVELOPER_ERROR` dev diagnostic in `googleSignIn.ts` + `docs/google-signin-setup.md` §3 now records the exact SHA to register and the mismatch. **Enablement + device verification are owner-side (Firebase console + rebuild).** |
| LB-030 | 🔴 P0 | Home / My Garden | Garden visuals render before the user has any plants | Garden UI is not gated behind `plants.length > 0` on every path (Round 1 `cbb93c8` aligned the *My Garden* empty-state copy, but Home/Garden still renders garden visuals before the first scan). | Gate all garden visuals behind `plants.length > 0`; until ≥1 plant exists show: empty-state illustration + "No plants yet" + CTA → "Scan your first plant". No garden renders until a plant exists. | 🔴 Pending | Re-open of Round 1 LB-030. |
| LB-031 | 🔴 P0 | Paywall / Premium | Premium page lacks clear purchase actions | On the build under QA the paywall has no clearly actionable purchase CTAs (Round 1 `c1b57d0` added `MOCK_PREMIUM_ENABLED`, but Monthly/Yearly/Continue/Restore are not surfacing/actionable in the QA build). | Ensure the Premium screen always shows: Monthly Plan CTA, Yearly Plan CTA, Continue button, Restore Purchases (where applicable), all actionable on dev/preview. Never a dead end. | 🔴 Pending | Re-open of Round 1 LB-031; real purchases still gated by BLK-4/5. |
| LB-032 | 🔴 P0 | Scan / Camera (recognition) | Recognition only reliable for leaf close-ups; whole plants often fail | Whole-plant framing yields low-confidence/failed results; capture UX does not steer users to optimal framing or validate the image before upload. AI provider (Plant.id) must not change. | Improve the **capture experience only**: capture guidance, pre-upload image validation (blur/too-far/dark), clearer UX before upload. Do not change the AI provider. | 🔴 Pending | Related to Round 1 LB-034 (same underlying recognition issue, different ID). |
| LB-033 | 🔴 P0 | Scan / Camera permission | First-time permission redirects to Settings instead of OS dialog | First camera-permission request routes straight to phone Settings rather than triggering Android's permission dialog; `canAskAgain` not honored on the first-run path. | First request → call OS `requestPermission` (Android dialog) → Allow → camera opens. Only route to Settings on permanent denial (`!canAskAgain`). | 🔴 Pending | Related to Round 1 LB-032 (camera permission). |
| LB-034 | 🔴 P0 | Scan / Camera state | Previous captured image persists after Back → reopen Scan | `capturedUri`/scan state is not cleared when leaving Scan via Back then reopening; reopening shows the old capture with Retake/Analyse (Round 1 `e8f50e4` added a focus listener but this path still persists state). | Clear `capturedUri` / quality / capture lock / scan store on blur **and** focus so Scan always opens on a fresh live camera unless the user explicitly saved the prior capture. | 🔴 Pending | Related to Round 1 LB-033 (camera state). |
| LB-035 | 🟠 P1 | Onboarding (vine) | Vine animation still reaches full growth too early | Route → growth-progress mapping is still not evenly distributed across onboarding screens; vine hits full growth with screens remaining (Round 1 `f6bb239` remap insufficient). | Remap growth to advance progressively across all onboarding screens; eliminate sudden jumps. Animation-timing change only. | 🔴 Pending | Re-open of Round 1 LB-028 (vine). |
| LB-036 | 🟠 P1 | Reminders / Notifications | Reminder notification reads like a developer test | Notification copy is a hardcoded placeholder ("LawnUp reminder test"); not production-quality or personalized. | Replace with production copy, personalized by plant/city/weather (e.g. "🌿 Time to water your Monstera.", "🌱 It's hot in Delhi today — your Money Plant may need water earlier.", "☔ Rain expected today — skip watering your Tulsi."). | 🔴 Pending | — |
| LB-037 | 🟠 P1 | Notifications (icon) | Notification icon not polished / off-brand | Adaptive + monochrome notification icon not reviewed for brand consistency. | Review and refine the adaptive icon and monochrome notification icon for brand consistency. | 🔴 Pending | May require native rebuild to verify. |
| LB-038 | 🟠 P1 | Scan (pre-capture guidance) | Pre-scan guidance unclear | No clear do/don't visual checklist before scanning, hurting capture success. | Add visual guidance before scanning (✔ entire plant · ✔ leaf close-up · ✔ flower · ✔ good lighting · ✖ blurry · ✖ dark · ✖ too far). Improve success rate without changing the AI. | 🔴 Pending | Complements LB-032 (capture UX). |
| LB-039 | 🟠 P1 | Scan / Processing | Analysis screen feels static | Loading does not convey real staged progress tied to analysis phases (Round 1 `de775f6` added 5 forward states but it still reads static). | Show staged progress mapped to real phases: Identifying plant… → Checking diseases… → Matching local weather… → Preparing personalized treatment… No fake delays. | 🔴 Pending | Re-open of Round 1 LB-036 (loading states). |
| LB-040 | 🟡 P2 | Bottom navigation | Active tab state not obvious enough | Active-tab emphasis still too weak to read at a glance (Round 1 `8a06ac9` added a highlight pill). | Strengthen the active-tab visual state for clearer visibility. | 🔴 Pending | Re-open of Round 1 LB-041 (bottom-nav active state). |
| LB-041 | 🟡 P2 | App-wide (empty states) | Empty states inconsistent | Not every empty screen has the standard illustration + explanation + CTA. | Audit every empty screen; each must contain an illustration, an explanation, and a CTA. | 🔴 Pending | Relates to LB-030 empty-garden state. |
| LB-042 | 🟡 P2 | Home (first-run) | New user doesn't immediately understand the app | First-run Home does not prioritize the key actions a new user needs. | Prioritize on first-run Home: Scan Plant, Add Plant, Ask Doc. Sage, Weather, Today's Care. | 🔴 Pending | — |
| LB-043 | 🟡 P2 | Home (weather card) | Weather card not contextual | Weather card shows raw values ("34°C") without contextual, city/plant-aware care guidance. | Render contextual copy, e.g. "It's 34°C in Delhi today. Water balcony plants earlier than usual." Reinforces the USP. | 🔴 Pending | Supports LB-046 USP audit. |
| LB-044 | 🟡 P2 | Paywall / Premium | Premium value proposition is generic | Premium messaging lists generic features instead of true differentiators. | Emphasize: Unlimited AI Diagnosis, Personalized Indian Care, Weather-aware watering, Disease history, Doc. Sage. | 🔴 Pending | Pairs with LB-031 (premium CTAs). |
| LB-045 | 🟡 P2 | Scan Result | No delight on successful identification | No subtle success feedback after a plant is identified. | Add lightweight success feedback (e.g. "🎉 Plant identified.", "🌿 Your Monstera has been added."). Keep animations lightweight. | 🔴 Pending | — |
| LB-046 | 🟡 P2 | App-wide (USP) | Personalized-care positioning not surfaced consistently | The positioning ("Generic plant advice doesn't grow healthy plants. Personalized care does." / "Care that's tailored to your plant, your city and today's weather.") is not surfaced naturally across surfaces. | Audit onboarding, Home, weather cards, reminders, scan results, Premium, and Doc. Sage; surface the USP naturally without making the UI feel repetitive. | 🔴 Pending | Cross-cutting; overlaps LB-036/043/044. |
