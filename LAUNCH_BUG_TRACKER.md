...
| **Current Commit** | `37f5b27` (QA Round 3 — LB-047…LB-051 all 🟡 Implemented; see §4.3) |
| **Owner** | Spriha Choudhary |
| **Last Updated** | 2026-06-20 — **QA Round 3 (device regression) implemented: LB-047…LB-051 (P0-1/P0-2/P0-3/P1-1/P1-2).** 2 are owner-side blocked (LB-048 Firebase SHA, LB-049 active Plant.id key). Awaiting next device regression pass — nothing 🟢 Verified yet (see §4.3) |
...
| LB-049 | 🔴 P0 | Scan / Plant.id | Scanner cannot be tested — identification fails | **Not credits, not an app bug — the Plant.id API key is inactive.** Probed the live endpoint (`GET /api/v3/usage_info`) → **HTTP 401 "The specified api key is not active"** (consistent with the documented key-leak/rotation). Secondary code bug: `mapToUserError` (useScanFlow) had no HTTP-4xx branch, so 401/402/429 fell through to a misleading "Something went wrong — please try again." Scanner UI / camera / gallery code is intact (LB-032/033). | `useScanFlow.ts`: added explicit branches — 401/403/"api key"/"not active" → "Plant identification is temporarily unavailable. Please try again later."; 402/429 (credits/rate-limit) → "…busy right now — please try again in a few minutes." Kept distinct from the user's own weekly scan limit (no false upgrade prompt). | 🟡 Implemented | `dee4148`. **Owner action:** set an **active** `PLANT_ID_KEY` (rotate per the leak) and rebuild. **Verify on device** (UI/camera/gallery + a real scan once a live key is set). |
| LB-050 | 🟠 P1 | Home (first-run) | First-time Home still reads like a garden-owner's screen | Reviewed the first-run layout: Home **already** gates all garden visuals (overview ring, dashboard, health grid) behind `hasPlants` and shows a focused first-run flow — Scan-your-first-plant hero → "OR START WITH" (Add Plant / Ask Doc. Sage) → weather/today's-care → "what one scan gives you" (LB-030/042). The one remaining garden-assuming surface was the pre-hydration loader copy **"Loading your garden…"**, shown during the brief unhydrated window to a brand-new (no-garden) user too. | `HomeScreen.tsx`: neutral pre-hydration loader copy → **"Getting things ready…"** (correct for both a returning user whose garden is loading and a new user with no garden). First-run hero/layout reviewed and left as-is (already correct). | 🟡 Implemented | `304e97b`. Pairs with LB-051 (hide Garden tab pre-first-plant). **Verify on device** (fresh account → Home shows the add-first-plant flow, no garden UI). |
| LB-051 | 🟠 P1 | Bottom nav (Garden) | Garden tab visible before the first plant | The Garden ("Plants") tab was always present, so a never-scanned user could land in an empty-garden surface — diluting the "add your first plant" onboarding. Decision (ticket delegated): hiding it is cleaner, **provided** the centered floating Scan button stays centered (it requires the symmetric 5-slot bar). | `MainTabNavigator.tsx`: `CustomTabBar` now reads `usePlantsStore` and, when `plants.length === 0`, renders the Plants slot as an **invisible spacer** (preserving the 5-slot grid + centered Scan) instead of the tab. The `Plants` screen stays registered so Add Plant / scan-result / PlantDetail navigation still works; the tab appears the moment the first plant lands. | 🟡 Implemented | `37f5b27`. Pairs with LB-050. **Verify on device** (fresh account: no Garden tab; save/scan first plant → Garden tab appears; Scan stays centered throughout). |
